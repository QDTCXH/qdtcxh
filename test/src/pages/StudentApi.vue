<template>
  <div class="showBox">
    ...
    <div class="section">
      <student-api-son :menuinfo="menuinfo" :clicks="changeSrc"></student-api-son>
      ...
      <student-api-con id="centerBox" :idxs="ind" :coninfo = "menuinfo"></student-api-con>
    </div>
  </div>
</template>
...
<script>
  ...
  import StudentApiSon from "../components/StudentApiSon";
  import StudentApiCon from "../components/StudentApiCon";
  export default {
    name: "show",
    components: {StudentApiCon, StudentApiSon, RightComponent, ConComponent, MenuComponent, HeaderComponent},
    data(){
      return{
        ind:'0',
        menuinfo:[
    ...
    created(){
      fetch("/static/Api.json").then(res=>{
        res.json().then(data=>{
          // console.info(data)
          this.menuinfo = data;
        })
